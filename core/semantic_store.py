...
Collection: ethos_value_prototypes
  One point per value (15 total). Each point is the L2-normalized centroid
  of 20-40 seed sentence embeddings for that value.
...
Usage:
  ...
  store.build_prototypes(seed_dict)  # seed_dict: {value_name: [sentence, ...]}
    ...
    with _LOCK:
        ...
        try:
            ...
            if _COLLECTION not in existing:
                ...
                _log.info("Created Qdrant collection: %s", _COLLECTION)
...
class SemanticStore:
    ...
    def prototype_count(self) -> int:
        """Return how many value prototypes are currently stored."""
        client = _get_client()
        if client is None:
            return 0
        try:
    ...
    def is_ready(self) -> bool:
        """True if Qdrant is connected and prototypes are built."""
